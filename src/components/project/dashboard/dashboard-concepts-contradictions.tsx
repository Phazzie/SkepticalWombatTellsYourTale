            ...
            {pendingConcepts.slice(0, 5).map((concept) => (
              <div key={concept.id} className="rounded-xl bg-app-surface-muted p-3 border border-app-border">
                ...
                <button
                  ...
                  aria-label={`Approve: ${concept.name}`}
                  className="text-xs text-neon-lime/70 hover:text-neon-lime mt-2 transition-colors focus-visible:outline-none focus-visible:ring-2 rounded px-1 -ml-1 focus-visible:ring-neon-lime"
                >
                  ✓ Approve
                </button>
              </div>
