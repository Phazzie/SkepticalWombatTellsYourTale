      ...
      <div className="rounded-2xl border border-app-border bg-app-surface p-5 shadow-app">
        ...
        {pendingConcepts.length === 0 ? (
          ...
          <div className="space-y-2.5">
            ...
            <Link href={`/project/${id}/concepts`} className="text-xs text-app-fg-muted hover:text-neon-lime transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neon-lime rounded px-1 -ml-1 inline-block">
              View all →
            </Link>
          </div>
        )}
      ...
      <div className="rounded-2xl border border-app-border bg-app-surface p-5 shadow-app">
        ...
        {openContradictions.length === 0 ? (
          ...
          <div className="space-y-2.5">
            {openContradictions.slice(0, 5).map((item) => (
              <div key={item.id} className="rounded-xl bg-neon-pink-dim p-3 border border-neon-pink/20">
                ...
                <button
                  ...
                  aria-label={`Mark explored: ${item.description}`}
                  className="text-xs text-neon-lime/70 hover:text-neon-lime mt-2 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neon-lime rounded px-1 -ml-1"
                >
                  ✓ Mark explored
                </button>
            ...
            <Link href={`/project/${id}/contradictions`} className="text-xs text-app-fg-muted hover:text-neon-lime transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neon-lime rounded px-1 -ml-1 inline-block">
              View all →
            </Link>
          </div>
        )}
