    ...
    <div className="mt-6 grid md:grid-cols-2 gap-6">
      <div className="rounded-2xl border border-app-border bg-app-surface p-5">
        ...
        {pendingConcepts.length === 0 ? (
          <p className="text-sm text-app-fg-muted">No pending concept names.</p>
        ) : (
          <div className="space-y-3">
            {pendingConcepts.slice(0, 5).map((concept) => (
              <div key={concept.id} className="rounded-xl border border-app-border bg-app-surface-muted p-3">
                <p className="text-sm text-green-300 font-medium">{concept.name}</p>
                <p className="mt-1 text-xs text-app-fg-muted">{concept.definition}</p>
                <button
      ...
      <div className="rounded-2xl border border-app-border bg-app-surface p-5">
        ...
        {openContradictions.length === 0 ? (
          <p className="text-sm text-app-fg-muted">No unresolved contradictions.</p>
        ) : (
          <div className="space-y-3">
            {openContradictions.slice(0, 5).map((item) => (
              <div key={item.id} className="rounded-xl border border-app-border bg-app-surface-muted p-3">
