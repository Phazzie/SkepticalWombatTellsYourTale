            ...
            {pendingConcepts.slice(0, 5).map((concept) => (
              <div key={concept.id} className="rounded-xl bg-app-surface-muted p-3 border border-app-border">
                ...
                <button
                  ...
                  className="text-xs text-neon-lime/70 hover:text-neon-lime mt-2 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neon-lime rounded px-1 -mx-1"
                >
                  ✓ Approve
                </button>
              </div>
            ...
            {openContradictions.slice(0, 5).map((item) => (
              <div key={item.id} className="rounded-xl bg-neon-pink-dim p-3 border border-neon-pink/20">
                ...
                <button
                  ...
                  aria-label={`Mark contradiction as explored: ${item.description.substring(0, 50)}...`}
                  className="text-xs text-neon-lime/70 hover:text-neon-lime mt-2 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neon-lime rounded px-1 -mx-1"
                >
                  ✓ Mark explored
                </button>
